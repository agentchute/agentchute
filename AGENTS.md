...
The pitch is intentionally narrow: agents sharing one inbox medium (typically running side-by-side in tmux panes on the reference CLI's shared, single-host filesystem — see AGENTCHUTE.md §2, multi-host network mounts are untested and out of scope) get a markdown-based mailbox so they stop copy-pasting handoffs by hand. That's the entire scope.
