...
- **E10 Test-environment hygiene.** When running Go tests from a lane that lives under the runner, strip the loop's environment first — leaked `AGENTCHUTE_*` variables cause false "serve lease fenced" failures: `env $(env | awk -F= '/^AGENTCHUTE_/{print "-u " $1}') go test ./...`. CI is the clean authority when local results look haunted. Scratch and proposal directories stay outside `go test ./...` reach (own `go.mod`, or a gitignored top-level dir).
