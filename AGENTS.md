...
The pitch is intentionally narrow: agents sharing one inbox medium (typically running side-by-side in tmux panes on the reference CLI's shared filesystem — single-host is the tested, supported configuration; a shared network mount across hosts works and is fail-closed safe, but is real-and-unverified rather than CI-tested, see AGENTCHUTE.md §2) get a markdown-based mailbox so they stop copy-pasting handoffs by hand. That's the entire scope.
