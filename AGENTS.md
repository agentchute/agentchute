...
- **E8 Drain and act within the working turn.** On a wake cue, check and handle the displayed batch, replying with exact references where needed. If another cue arrives or a continue/commit/finish gate reports unread or malformed mail, re-check and handle the delta in the same working turn. Recognize replayed identities and act idempotently. Do not check repeatedly without cue or gate evidence. Guarded lanes leave archival and latch clearing to their normal ordered hook. Hookless lanes explicitly acknowledge completed batches. Inspect reply obligations and the relevant gate before reporting completion. Additional unread/malformed mail requires another consume/act pass, not a mandatory turn boundary. Other gate blockers retain their existing rules. Stale assignments still require sender confirmation. Old-binary fallback: a denied re-check requires the verified normal end-of-turn hook and its blocked-finish continuation. Do not bypass the latch or claim unfinished work complete. Document this as a compatibility limitation, not the intended flow.
