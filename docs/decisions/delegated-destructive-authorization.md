...
Recommendation: **DO NOT BUILD an in-repo grant mechanism.** Keep direct operator confirmation as the authorization boundary unless a deployment supplies an operator signing identity that no lane can access.
...
> No fact supplied through an inbox, or written by a process with the same authority as a lane, can authorize a destructive or irreversible action.
...
1. A PM or peer may assign the task over agentchute. The lane may inspect, plan, implement reversible preparation, and report the exact destructive step it expects to need.
2. The lane stops before the destructive or irreversible step. The PM routes the need to the operator.
...
Authorization stays concrete rather than becoming a capability language. A valid direct confirmation names:

- the action, such as delete local branches, delete remote refs, push a branch, publish a release, change a repository setting, or contact an external service;
- the repository, service, or filesystem root affected; and
- the exact targets, such as branch names, ref names, PR number, tag, release version, setting, endpoint, or path.

`AUTHORIZATION:` remains the recommended presentation, not a token the binary parses. For example:

```text
AUTHORIZATION: push branch decision/delegated-destructive-authorization at <sha> to origin; open a PR from that branch to main.
```

Broad descriptions such as `clean up stale branches`, `do whatever is needed`, or `apply this grant` are not sufficient. Shell syntax alone is also insufficient when it contains unresolved globs, substitutions, or variables; the authorization names the resolved objects.

## Lifetime, expiry, and revocation

There is no durable grant to expire or replay.

- Confirmation applies only to the named action and targets in the current directly authenticated task context.
...
- An inbox instruction may always narrow, pause, or cancel work. It cannot extend, restore, or transfer authorization.
...
Under this decision, control of an inbox cannot:
...
- broaden the action or targets named in a direct confirmation;
...
Inbox control can still request work, misrepresent facts, cause denial of service, and exploit a recipient that violates Prompt Safety. A process with the pool user's general shell access can bypass the coordination layer and act directly; that stronger threat is outside agentchute's security model.
