...
4. ~~**The wrappers-null warning is now membership-only.** Renamed `hookRefreshSkipWarning` → `wrappersUnrecordedWarning` (`internal/cli/update.go`); it no longer claims hooks will not refresh, and no longer prescribes `hooks install` as the safety repair — that repair is now automatic. It still fires when a resync's recorded wrapper list is empty while hook files exist, because the pool's membership bookkeeping being out of date is still worth surfacing.~~ **SUPERSEDED 2026-08-12**: update now repairs the bookkeeping itself — no NOTE prescribing a manual `setup --wrappers <list>` follow-up. `setup --wrappers none` records a distinguishable non-nil empty list (`"wrappers": []`); only a nil list (JSON null — state predating that recording) with installed hook files gets membership adopted from `installedHookWrappers` and written directly to `setup.json` (`internal/cli/update.go`), before the already-up-to-date early exit can return. A recorded `[]` is never overridden: hook files legitimately exist outside membership via `hooks install --scope repo`, so file presence does not prove membership (codex FIX on the first cut of this change, which wrongly assumed it did).
