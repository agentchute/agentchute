...
4. ~~**The wrappers-null warning is now membership-only.** Renamed `hookRefreshSkipWarning` → `wrappersUnrecordedWarning` (`internal/cli/update.go`); it no longer claims hooks will not refresh, and no longer prescribes `hooks install` as the safety repair — that repair is now automatic. It still fires when a resync's recorded wrapper list is empty while hook files exist, because the pool's membership bookkeeping being out of date is still worth surfacing.~~ **SUPERSEDED 2026-08-12**: update now repairs the bookkeeping itself — no NOTE prescribing a manual `setup --wrappers <list>` follow-up. every current setup run records a non-nil wrapper list — explicit `--wrappers none` and a zero-detection `--wrappers all` both record `"wrappers": []` — so null stays legacy-only; only a nil list (JSON null — state written before this) with installed hook files gets membership adopted from `installedHookWrappers` and written directly to `setup.json` (`internal/cli/update.go`) after target/tag validation succeeds but before the already-up-to-date early exit can return. A recorded `[]` is never overridden: hook files legitimately exist outside membership via `hooks install --scope repo`, so file presence does not prove membership (codex FIX on the first cut of this change, which wrongly assumed it did).
...
On a verification failure, `applySetup` returns before Phase 3 (the destructive runtime reset), so it never forces a restart into a hook that would fail on its very next invocation. What that buys depends on the caller. A **direct `agentchute setup`** invocation has invalidated no serve lease yet at this point, so its supervisors are left genuinely un-fenced. An **`agentchute update` resync** is different: `cmdUpdate` invalidates every serve lease *before* it re-execs `setup` (`internal/cli/update.go`, well before the resync call), so supervisors are already fenced by the time this phase could fail — skipping Phase 3 there mainly avoids a redundant local-runner-stop/state-clear. The update path's real protection is `cmdUpdate`'s existing resync-failure handling: it already treats any non-zero `setup` exit as a failed resync, prints the manual fix command, and returns non-zero without ever reaching the final restart-success line — unchanged by this PR, and now exercised against this specific failure shape by `TestUpdate_ResyncVerificationFailurePreventsFinalRestartBanner` (`internal/cli/update_test.go`).

Out of scope: ~~auto-repairing `setup.json`'s `wrappers: null` (unchanged from the prior decision — operator re-records at the next planned restart)~~ (done 2026-08-12 — see the item-4 supersession above); any change to `--wrappers none`'s membership semantics; protocol or wire changes; new subcommands.
