...
4. **Own the checkout, own the cleanup — same turn, before the verdict, every lane.** If the review needed a separate pinned-SHA checkout (not a diff read from your current tree), remove it before delivering the verdict: `git worktree remove <path>`. This binds regardless of wrapper — 27 stale `review-pr*` checkouts accumulated from BOTH Claude and Codex review paths before this line existed anywhere every lane actually reads. Skip only when the review used no separate checkout. Do NOT also delete a local branch here: a review checkout is normally a detached-HEAD pinned SHA with no branch at all, and branch deletion is its own operator-reserved action (Rule 5; `repo.shared.mutate`) that needs proof no unique work would be lost — this step is a git-mechanics cleanup, not a license to force-delete.
