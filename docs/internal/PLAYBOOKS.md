...
1. On a cue, check and read the whole batch.
2. Batch the verification needed for the assignment.
3. Act and reply using exact references; recognize replayed identities.
4. A new cue or unread/malformed gate result triggers another consume/act pass within the same working turn. Revalidate any review target affected by new instructions.
5. Before reporting completion, inspect pending obligations and the relevant gate. Guarded lanes commit through their normal end hook; hookless lanes explicitly acknowledge first. Neither path bypasses a blocker.
6. End the turn when the work calls for it. Do not speculate by polling after closeout.

Coordinators: batch follow-ups to a lane that is mid-task; review only committed, pinned objects; never read or amend another lane's uncommitted working tree.
...
5. **Re-check immediately before you send, not just before you start** — with commands that actually run mid-turn. A verdict is a snapshot, and a long review turn means the branch may have moved under it. Right before delivering: re-run `gh pr view <n> --json headRefOid`, then run the read-only `agentchute gate --before continue --as <id>`. If that gate reports unread or malformed mail, check again in this same working turn, handle the new instructions, refresh the pinned target, then send — do not end the turn solely for that blockage. Other gate blockers retain their existing rules. Older binaries still deny `check` while latched; that fallback is a compatibility limitation — use the verified end-of-turn hook, do not bypass the latch. Review committed, pinned objects; do not amend another lane's unfinished working-tree changes. Never build a finding on a peer's *local* worktree state either — a commit sitting in their checkout but not on the PR head may simply not have been pushed yet, and by the time you finish writing it usually has been. Both failures happened in one turn on PR #146: a FIX went out for something already fixed and pushed, criticising a push-discipline gap that had already closed.
