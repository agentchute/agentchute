...
Latest release: **`v0.10.1`** — edges and honesty (patch): runner diagnostics → `state/<id>/runner.log` (kills the raw-mode screen smears), `ack` commits unconditionally + reports (hook mode via `--quiet`), `doctor` surfaces crash-orphaned `.tmp_*` files across all four origins, and the spec gained **§15 Security Considerations** (bodies are data; wrapper confirmation rule; marker v22) plus spec–implementation honesty fixes (send crash-window, `from`-via-filename, reserved `v:` field, NFS mount requirements, skew, backpressure, seq recovery). Additive analysis items deferred to v0.11.0. Prior: v0.10.0 — the finish line, **Protocol v2 declared STABLE**. Built on v0.9.x subtraction + v0.8.0 "simple again".
