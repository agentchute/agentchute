...
Coordination is **pull-only**: senders only ever write files and never poke a recipient. A loopless wrapper runs under the runner (`agentchute serve`, launched via `ac serve <wrapper>`) — a per-agent PTY supervisor that polls the agent's own inbox and injects a `check inbox` cue. There is no watchdog, no reachability cache, and no tmux/herdr wake adapters; those were removed. (v2.5 in flight, not yet released as of this line's last edit — see `docs/decisions/agentchute-v2.5-implementation-plan.md` and AGENTCHUTE.md for the current wire truth, not this paragraph: message identity is moving to a timestamp+random-suffix tuple, presence is moving to registration-row `last_seen` with no separate presence file, and a lazy sweep replaces manual deregistration.) Reply obligations are asker-owned (`.owed`). Id-uniqueness rides a serve lease + fencing token. The protocol's invariants ship as an executable suite in [`conformance/`](conformance/).
