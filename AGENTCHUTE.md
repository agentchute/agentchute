...
A runner may have at most one queued or waiting wake attempt for its active pending-mail period. While waiting for an injection opportunity, it MUST continue observing its own raw inbox. An observed empty inbox retires the waiting attempt without a cue, even if the wrapper never becomes idle. Retirement releases the pending-wake state as part of cancelling or completing that actual attempt; clearing a flag while leaving a waiter alive is insufficient.

Subsequent mail starts a new pending period. An old attempt MUST NOT inject into, or clear pending state belonging to, a newer period. Immediately before delivery, recheck pending mail and attempt ownership. Malformed inbox files still require a cue. A failed listing is not evidence of an empty inbox. Successful PTY writing establishes only that bytes were written, not that the wrapper acted on them.

Remote runners use successful hub tick pending/skipped results. Channel loss is not an empty inbox and retains the existing channel-loss behavior. Activity and wake state stay on the joining machine; no new mailbox or wake operation is added to the hub protocol.
...
A row's age compares its `last_seen` against the reader's clock under the same NTP-loose assumption as §5.4: clock skew between reader and writer shifts perceived freshness in either direction. Because `last_seen` serialises at second precision while a reader's clock does not, a row may legitimately read as slightly AHEAD of the reader; a sweep MUST treat a `last_seen` up to **2 seconds** in the future as fresh rather than as maximally stale, and MUST NOT extend that tolerance far enough to make a badly future-dated row permanently sweep-immune. The reference CLI uses exactly 2s: one second is the granularity the serialisation guarantees, the second is slack for the pass itself. Stale or absent ⇒ **not-alive** — never an error; an unregistered or long-gone agent simply reads not-alive. This is the dead-mailbox detection: "came back days later, one agent never returned" surfaces as a stale row.
...
`E_HUB_PINNING_UNVERIFIED` is the third outcome and asserts neither of the other two. It is
client-emitted and **retriable**: the client's probe could not run — ssh itself failed to
start, timed out, or exited 255 — so nothing about the host was observed. A remote exit 127
is NOT this case; that probe ran and found no nonce, which is what `E_HUB_NO_BINARY` reports.
A code's name is its claim, and when the probe never ran neither neighbour's claim is known.
...
**The guard latch and its PreToolUse-family deny list are a short, best-effort SUBSET of denied commands, not a comprehensive list and not a security boundary.** Its scope is mail-pipeline integrity, not general scope-expansion protection: the causal path between claiming mail (`check`) and committing it (`turn-end`) — commands that could destroy the claim (`rm -rf`; `curl`/`wget`, which can move an arbitrary payload off the command line and reach the rest with no denied token of its own) or disable/bypass the committing handler itself (`agentchute ack`/`turn-end`/`update`/`setup` under any spelling, and `clean` except when scoped to `--owed`), plus direct writes to the hook config files. It deliberately does NOT deny `git push`, `git tag`, `gh release`, `gh pr merge`, `ssh`, `scp`, or anything else outside that causal path (guard-latch-livelock fix — those were cut, not omitted by oversight): every wake cue mandates checking inbox at turn start, which arms the latch, so denying an implementer's own push/PR/tag denied exactly the action the turn existed to perform, a livelock hit live in this program's own fleet. The latch is a plain file under the agent's own per-agent state directory, and the deny list is command-text pattern matching — both bypassable directly by the same cooperating, same-UID process the guard is meant to slow down: deleting `state/<id>/guard.latch` (or any other state file) is not prevented by any file permission (0700 protects against *other* users, not the file's own owner), and a differently spelled, quoted, or dispatched command can evade the pattern match. Treat a guarded lane as a speed bump against accidental mail-integrity damage, never a barrier against a determined adversarial agent or operator, and never as protection against a scope-expanding action generally.

**Same-session re-check.** Further claiming/display is allowed while the current session's latch is armed. Re-checking MUST NOT clear the latch or derive its state from claimed-directory emptiness. It retains/arms the latch, recovers residue, and processes new inbox work. Only the session's ordered end-of-turn handler archives and clears that latch. A lifecycle record or Git commit is not a substitute and grants no additional authority. The guard remains best-effort mail-integrity protection; existing mixed-hook-trust recovery requirements remain in force.
