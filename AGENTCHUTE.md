...
**Compatibility fields (one release, accepted but de-emphasized):** `message_id` is no longer emitted (removed in v0.9.0); the identity is `(to,from,seq)` and reply threading rides `in_reply_to` (the canonical `(to,from,seq)` ref). A `message_id` on an older in-flight message is still tolerated on read (ignored — never the identity). `to`, `task`, and `status` are accepted if present but are no longer part of the normative envelope (`to` is encoded by location; `task`/`status` are body conventions).
