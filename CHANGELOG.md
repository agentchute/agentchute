...
**Operator note**
- Codex 0.154 TUI: set `[tui] whimsy = false` and restart the lane. In the affected configuration — Astra composer stars enabled in a true-color terminal — whimsy redraws (upstream [openai/codex#42842](https://github.com/openai/codex/issues/42842), about every 150 ms while idle on the probe taken) starve the idle window the runner uses to inject a cue; both codex lanes observed on 2026-09-17 were affected. `tui.animations = false` also stops them but freezes the Working timer.
...
**One new error code** — `E_HUB_PINNING_UNVERIFIED`, client-emitted, retriable — is the only registry surface here. A joined machine and its hub still need matching versions; nothing else on the wire moved.

**Say what actually failed**
- `update` quotes the re-sync child's last words in the WARNING block instead of reporting `exit status 1` alone. The child's stderr always reached the terminal — it arrived a dozen lines above a warning that named only an exit status, so the line an operator's eye lands on carried no cause. The child still streams live; a bounded tail is kept alongside it ([#184](https://github.com/agentchute/agentchute/pull/184), closes [#183](https://github.com/agentchute/agentchute/issues/183)).
- The pinning probe has a third answer. A probe that **could not run** now reports UNVERIFIED rather than PINNED — `doctor` warns with the reason instead of printing the all-clear for a check that never happened, and exit 127 gains `E_HUB_PINNING_UNVERIFIED` (retriable) naming both causes when it cannot tell them apart. The discriminator is whether **ssh itself** failed — a local exec failure, the timeout, or ssh's reserved exit 255 — because a pinned host with a missing binary exits 127 and that probe ran perfectly ([#195](https://github.com/agentchute/agentchute/pull/195)).
- A multiplex master is reported reaped only when the socket is **provably gone**. `control socket connect` is the prefix ssh prints for every connect failure; only "No such file or directory" proves absence. "Permission denied" may be a live master owned by another account, and "Connection refused" is indistinguishable from a live master whose listen backlog is full. A migration's reap failures are now reported rather than discarded ([#194](https://github.com/agentchute/agentchute/pull/194)).
- Two silent give-ups: a `git` failure of any kind no longer reads as "not a repository" when deciding whether the control-repo pointer got excluded, and the stop-a-process path refuses when it cannot compare a recorded host against this machine's name ([#196](https://github.com/agentchute/agentchute/pull/196)).

**Fail closed where it used to fail open**
- `wipe` refuses when the serve-claim directory is **unreadable**. The `ReadDir` error was discarded, so an unreadable `state/` produced an empty list — indistinguishable there from "no claims at all" — and the wipe proceeded ([#193](https://github.com/agentchute/agentchute/pull/193)).
- The post-wipe rescan reports what it **could not read** separately from what it found. "I looked and found nothing" and "I could not look" are opposite answers to the question it asks, and the caller was handed the reassuring one ([#193](https://github.com/agentchute/agentchute/pull/193)).
- `AcquireServeLease` refuses when this machine cannot determine its own hostname. The error was discarded and the host recorded as `""`, so a frozen-but-alive lane on this very machine looked cross-host and had its id stolen. An empty host already on disk now means "cannot prove it is elsewhere" ([#193](https://github.com/agentchute/agentchute/pull/193)).

**Joins that converge**
- `hub join` records a host-key fingerprint via `ssh-keyscan` when `known_hosts` yields nothing — the fallback migration already had — and stops discarding the read error. Without one, that hub could never be migrated, and the operator met it later as a duplicate-key refusal recommending `--replace-key`, which was the wrong remedy ([#185](https://github.com/agentchute/agentchute/pull/185), closes [#164](https://github.com/agentchute/agentchute/issues/164)).
- A key whose `.pub` never landed — an interrupted mint — is repaired from the `ssh-keygen -y` output the passphrase probe already runs, rather than looping. Regenerating rather than minting is the safe half: a replacement would strand a credential the hub may already have authorized. `mintHubKey` now checks for the `.pub` it claims ([#191](https://github.com/agentchute/agentchute/pull/191), closes [#166](https://github.com/agentchute/agentchute/issues/166)).

**Two CI flakes that were not flakes**
- The sweep could delete a **live lane's** registration row. Registrations serialise `last_seen` at second precision while the sweep captures `now` at full precision, so a heartbeat crossing a second boundary read as maximally stale — in the very window the under-lock re-check exists to protect. Slightly-future is now fresh; far-future stays sweepable ([#197](https://github.com/agentchute/agentchute/pull/197), closes [#175](https://github.com/agentchute/agentchute/issues/175)).
- A signal arriving mid-write no longer cuts a control frame in half. sshd SIGHUPs a forced command when its channel goes away, so an ordinary disconnect produced a corrupt frame and "the hub looks broken" for a hub that was fine. An asynchronous close now waits a bounded ~250ms for an in-flight write — bounded, because those closers are also the unwedging mechanism ([#198](https://github.com/agentchute/agentchute/pull/198), closes [#176](https://github.com/agentchute/agentchute/issues/176)).

**The guard stops denying the fix it recommends**
- `clean --owed` is allowed while the guard latch is held. `check` armed the latch and printed "prune with: `agentchute clean --owed`" in the same breath, and the latch denied it; `turn-end` clears the latch at the end of a turn, so a lane never had a moment where it both knew about the obligation and could act. `clean --mailbox`, which deletes a peer's inbox, stays denied ([#188](https://github.com/agentchute/agentchute/pull/188), closes [#174](https://github.com/agentchute/agentchute/issues/174)). A sentence in the enrollment blocks was repaired after that change ([#192](https://github.com/agentchute/agentchute/pull/192)).

**The spec says what it does not enforce**
- §13.9a stated the write-lock MUST and what happens in a compliant world; it said nothing about what is true when the MUST is violated, which is what a reader of a structural-sounding promise needs. It now says the requirement is enforced at **acquisition**, not at write time, and cannot be enforced at write time — a descriptor follows the inode, so a writer that never took the lock is invisible to the lock. Both consequences are named as accepted: a violator that re-resolves its path after the freeze is **orphaned**, one holding a descriptor from before the freeze is **destroyed** with the frozen tree. Neither is reachable from agentchute's own paths; both stay reachable for a binary older than the contract and for any process outside agentchute — which is why an internal barrier was rejected rather than deferred, and why the closing sentence names the structural repair instead ([#200](https://github.com/agentchute/agentchute/pull/200), see [#179](https://github.com/agentchute/agentchute/issues/179)).

**The local ritual matches the gate that judges it**
- `tools/test.sh` runs `go test -race`, which `ci.yaml` and `release.yaml` always did. The ritual every lane is told to run before pushing was strictly weaker than the gate judging the push, and weaker in exactly the class that is invisible locally ([#187](https://github.com/agentchute/agentchute/pull/187), closes [#153](https://github.com/agentchute/agentchute/issues/153)).
- A test now fails when the two disagree, so the next divergence surfaces there instead of on a runner — or after a tag ([#189](https://github.com/agentchute/agentchute/pull/189)).

**Known follow-ups:** [#179](https://github.com/agentchute/agentchute/issues/179) (keep lane state out of the hub-id-keyed directory — the structural repair; §13.9a states the ceiling until then), [#180](https://github.com/agentchute/agentchute/issues/180) (migrations unreachable in the sshd harness), [#190](https://github.com/agentchute/agentchute/issues/190) (should an unanswered reply obligation expire silently), [#199](https://github.com/agentchute/agentchute/issues/199) (a contended lock acquire costs a multiple of 25ms), plus the UX and housekeeping tier: [#170](https://github.com/agentchute/agentchute/issues/170), [#171](https://github.com/agentchute/agentchute/issues/171), [#177](https://github.com/agentchute/agentchute/issues/177), [#178](https://github.com/agentchute/agentchute/issues/178), [#152](https://github.com/agentchute/agentchute/issues/152), [#157](https://github.com/agentchute/agentchute/issues/157), [#63](https://github.com/agentchute/agentchute/issues/63). [#167](https://github.com/agentchute/agentchute/issues/167)'s **entire current inventory is fixed** — its original four across v1.6.0 and this release, and every item its second, read-only sweep enumerated: the fail-closed wipe and lease trio, the reap pair, the pinning three-way, and the two silent give-ups. It stays open on purpose. Its deliverable was always the defect CLASS — a check reporting success without having verified anything — and closing on a count would close on the instances that happened to be found.
...
**Known follow-ups at release:** [#164](https://github.com/agentchute/agentchute/issues/164) (join records no host-key fingerprint when `known_hosts` yields nothing) *(closed in v1.6.1 by [#185](https://github.com/agentchute/agentchute/pull/185))*, [#175](https://github.com/agentchute/agentchute/issues/175) (`internal/loop` test races) *(closed in v1.6.1 by [#197](https://github.com/agentchute/agentchute/pull/197))*, [#176](https://github.com/agentchute/agentchute/issues/176) *(closed in v1.6.1 by [#198](https://github.com/agentchute/agentchute/pull/198))*, [#179](https://github.com/agentchute/agentchute/issues/179), [#180](https://github.com/agentchute/agentchute/issues/180). The heading is what was true at the v1.6.0 tag; the annotations keep it from reading as open work on `main`.
...
**Install/upgrade reliability** — a same-day investigation by two independent lanes empirically reproduced the stale-hook failure modes below before their fixes were written; follow-up review of those fixes then added the symlink safety guard and the CI coverage.
...
**Owner exception (logged per the policy's mechanism):** the ratified dogfood window (≥72h + 9 observable criteria) was closed early by owner decision at the tag. Baseline verification at window-open was fully green — pool on v0.11.8 release binaries installed via the real paths, `v: 2` live on all five lanes at capture, doctor clean, conformance green against the release tree — and 4 of 9 criteria were already satisfied. During the window one lane (gemini) cycled back to an absent-`v` legacy registration — which put the pool in exactly the mixed absent-`v`/`v:2` state the new semantics were designed for (and that the exit criteria wanted exercised): coordination across the boundary continued unaffected, absent-`v` stayed silent, and no warning storm occurred. The remaining observational criteria continue as ordinary post-release monitoring on the live pool that builds agentchute with agentchute.

**Not in 1.0, by decision (the rejects are the statement):** native Windows; signing beyond checksums; non-filesystem transports in the reference CLI; broadcast/routing/coordinator agents; maintained SDKs or a maintained second implementation (the language-neutral vectors are the deliverable — a 269-line disposable Python proof already passes them); packaging/distribution channels; supervision/rollback tooling; default retries or exactly-once; dashboards, daemons, config systems. This is a protocol's 1.0, not a product's.
...
> *Measurement note (added at 1.0): −8,281 is the sum of the nine per-PR diffs, which double-counts intermediate churn. The tag-range diff `git diff --shortstat v0.8.8 v0.9.0` gives +1,370/−9,632 = **net −8,262**, which is the canonical, one-command-reproducible figure used in the 1.0 launch materials.*
