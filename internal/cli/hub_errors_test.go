...
func TestHubErrorCatalogGoldenTexts(t *testing.T) {
	...
	rows := []struct {
		...
		{"E_FENCED", hubFencedError("codex").Error(), "serve: this lane was fenced out (lease reclaimed — likely a newer serve for codex, or a hub update). Restart this lane: ac serve codex"},
		...
		{"join busy", hubJoinBusyError("/home/alex/.agentchute/hub/.locks/3fa8c21b90de.lock").Error(), "hub join: another agentchute hub join/rotate is already running for this hub (lock ~/.agentchute/hub/.locks/3fa8c21b90de.lock). Wait for it to finish and re-run."},
