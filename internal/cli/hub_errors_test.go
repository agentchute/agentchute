...
func TestHubErrorCatalogGoldenTexts(t *testing.T) {
	...
	rows := []struct {
		...
		{"blocked by migration", hubLockBusyError("/home/alex/.agentchute/hub/.locks/3fa8c21b90de.lock", hubLockShared).Error(), "hub: this hub is being migrated right now (lock ~/.agentchute/hub/.locks/3fa8c21b90de.lock held exclusively). This command writes into the directory being moved, so it will not run until the migration finishes; re-run in a moment."},
