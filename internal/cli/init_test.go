...
func TestInitFreshEmpty(t *testing.T) {
	...
	expectAction(t, plan, "CLAUDE.md", "create v31")
	expectAction(t, plan, "CODEX.md", "create v31")
	expectAction(t, plan, "GEMINI.md", "create v31")
	expectAction(t, plan, "GROK.md", "create v31")
	expectAction(t, plan, "AGENTS.md", "create v31")
...
func TestInitPrependsBlockWhenNoMarker(t *testing.T) {
	...
	expectAction(t, plan, "CLAUDE.md", "prepend v31")
	applyAll(t, plan)
	...
	if !strings.Contains(string(got), "agentchute-enrollment v31 begin") {
...
func TestInitReplacesDriftedV1Content(t *testing.T) {
	...
	expectAction(t, plan, "CLAUDE.md", "replace v1→v31")
	applyAll(t, plan)
...
func TestInitUpgradesV11EnrollmentBlockToV13(t *testing.T) {
	...
	expectAction(t, plan, "CODEX.md", "replace v11→v31")
	applyAll(t, plan)
...
func TestInitLeavesNewerVersionAlone(t *testing.T) {
	root := t.TempDir()
	future := "<!-- agentchute-enrollment v32 begin -->\nfuture\n<!-- agentchute-enrollment v32 end -->\n"
...
// TestInitUpgradesV30EnrollmentToV31 is the enrollment-bump lock (introduced
...
// v30→v31"), a distinct action string from planEnrollmentFile's same-version
// "replace vN drift" branch, so the two never get confused in plan output.
// TestInitLeavesNewerVersionAlone (this file, now pinned to a v32 fixture)
...
// rewriting — the same generic branch a literal v30 binary against a v31 file
// would take, which cannot be built as a second binary within this same test
// run.
func TestInitUpgradesV30EnrollmentToV31(t *testing.T) {
	root := t.TempDir()
	old := "<!-- agentchute-enrollment v30 begin -->\nstale v30 content\n<!-- agentchute-enrollment v30 end -->\n\nMy notes.\n"
	...
	for _, a := range plan.Actions {
		...
		if a.Action != "replace v30→v31" {
			t.Errorf("action = %q, want %q", a.Action, "replace v30→v31")
		}
	...
	if !strings.Contains(string(got), "agentchute-enrollment v31 begin") {
		t.Errorf("CLAUDE.md was not upgraded to v31:\n%s", got)
	}
	if strings.Contains(string(got), "stale v30 content") {
		t.Errorf("CLAUDE.md still contains stale v30 content after upgrade:\n%s", got)
	}
	if !strings.Contains(string(got), "My notes.") {
