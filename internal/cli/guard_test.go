...
func TestGuardDirectSendDataSinkException(t *testing.T) {
	cases := []struct {
		...
		{"identity env quoted", `agentchute send --from "$AGENTCHUTE_AGENT_ID" --to claude-code --body 'done'`, false},
		{"identity env braced", `agentchute send --from "${AGENTCHUTE_AGENT_ID}" --to claude-code --body 'done'`, false},
		{"identity env unquoted", `agentchute send --from $AGENTCHUTE_AGENT_ID --to claude-code --body 'done'`, false},
		{"identity env longer var name", `agentchute send --from "$AGENTCHUTE_AGENT_IDX" --to claude-code --body 'done'`, true},
		{"identity env adjacent to token var", `agentchute send --to claude-code --body "$AGENTCHUTE_AGENT_ID$AGENTCHUTE_SERVE_TOKEN"`, true},
